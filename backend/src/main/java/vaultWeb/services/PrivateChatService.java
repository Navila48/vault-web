...
public class PrivateChatService {
    ...
    @Transactional
    public Long createGroupFromChats(List<Long> privateChatIds, String groupName, String description, String currentUserName) {
        User creator = userRepository.findByUsername(currentUserName).orElseThrow(() -> new UserNotFoundException("User not found: " + currentUserName));
        ...
        for (Long privateChatId : privateChatIds) {
            PrivateChat privateChat = privateChatRepository.findById(privateChatId).orElseThrow(() -> new PrivateChatNotFoundException("No private chat with this id " + privateChatId));
            ...
            if (!(isOwnerIsUser1 || isOwnerIsUser2)) {
                throw new UnauthorizedException("User is not allowed to create group with chat where he didn't participate in " + privateChatId);
            }
